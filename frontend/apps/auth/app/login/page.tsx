...
import { createUserSession, clearAllSessions } from '@commercialapp/ui';
...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginFormValues) => {
    ...
    try {
      ...
      // Clear any existing cache first to ensure fresh session
      clearAllSessions();
      
      // Create session with username only (no accountCode storage)
      createUserSession(data.username);
      
      // Redirect to chartering page (no accountCode in URL)
      const url = `${process.env.NEXT_PUBLIC_CHARTERING_URL || 'http://localhost:3000'}/?from=auth`;
      router.push(url);
